"""OCR-backed text extraction utilities."""
...
def extract_text(image_path: PathLike) -> list[dict[str, Any]]:
    ...
    Raises:
        ...
        RuntimeError: If the selected OCR backend is unavailable or cannot
            initialize.
    """
    source = Path(image_path)
    image = _read_image(source)

    backend = os.getenv("OCR_BACKEND", "paddle").strip().lower()
    engine = None if backend == "tesseract" else _get_ocr_engine()
    ...
    if not _needs_retry(primary):
        if _needs_small_text_retry(primary):
            ...
            return _merge_declaration_retry(
                primary,
                _restore_retry_coordinates(
                    _run_selected_ocr(backend, engine, retry_image),
                    scale,
                    y_offset,
                ),
            )
...
def _run_ocr(engine: Any, image: np.ndarray) -> list[dict[str, Any]]:
    """Run the existing PaddleOCR engine."""
...
def _run_tesseract_ocr(image: np.ndarray) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(
            "Tesseract OCR adapter is unavailable. Install pytesseract."
        ) from exc
...
def _result_quality(
    ...
    anchors = sum(
        any(
            term in str(item.get("text", "")).casefold()
            for term in _DECLARATION_TERMS
        )
        for item in results
    )
...
def _is_duplicate_detection(
    ...
    return bool(text) and any(
        text == _normalized_detection_text(item.get("text"))
        for item in existing
    )


def _is_meaningful_retry_detection(
    candidate: Mapping[str, Any],
    retry: list[dict[str, Any]],
) -> bool:
...
def _is_address_fragment(text: str) -> bool:
    return bool(
        re.search(
            r"\b(?:floor|road|street|marg|lane|estate|sector|building|"
            r"court|p(?:in)?\s*[-:]?\s*\d{6})\b",
            text,
            re.IGNORECASE,
        )
    )
...
def _nearby_entity_declaration(
    candidate: Mapping[str, Any],
    retry: list[dict[str, Any]],
) -> bool:
    ...
    for item in retry:
        ...
        if not _ENTITY_CUE_PATTERN.search(
            str(item.get("text", ""))
        ):
            continue
        ...
        aligned = overlap >= (
            min(right - left, other_right - other_left) * 0.2
        )
        ...
        if aligned and vertical_gap <= max(
            candidate_height,
...
def _read_image(source: Path) -> np.ndarray:
    if not source.is_file():
        raise FileNotFoundError(
            f"Input image does not exist or is not a file: {source}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Unable to read input image: {source}"
        ) from exc
...
def _get_ocr_engine() -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "in a Python environment supported by PaddlePaddle before "
            "using OCR."
        ) from exc
    ...
    try:
        ...
        raise RuntimeError(
            "Unable to initialize the PaddleOCR engine."
        ) from exc
...
def _parse_legacy_results(
    ...
    if (
        isinstance(raw_results, (list, tuple))
